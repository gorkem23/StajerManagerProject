...
function EditStajerModal({ stajer, onClose, onSuccess, show = false }) {
  const {
    formData,
    setFormData,
    dropdownData,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await stajerService.update(stajer.stajerID, stajerData)
      
      if (response.success) {
        showToast('Stajer başarıyla güncellendi.', { type: 'success' })
        onSuccess?.()
        baseHandleClose()
      } else {
        showToast(response.message || 'Stajer güncellenemedi.', { type: 'error' })
      }
    } catch (err) {
      ...
      const errorMessage = errorData?.message || 
                          ...
                          err.message || 
                          'Stajer güncellenirken bir hata oluştu.'
      
      showToast(errorMessage, { type: 'error' })
      console.error('Error updating stajer:', err)
    } finally {
      setLoading(false)
    }
  }

  if (!stajer || !show) return null
  ...
  return (
    <div 
      ...
      id="editStajerModal" 
      tabIndex="-1" 
      aria-labelledby="editStajerModalLabel" 
      aria-hidden="true"
      ref={modalRef}
    >
      <div className="modal-dialog modal-dialog-centered modal-lg">
        <div className="modal-content">
          <div className="modal-header">
            <h1 className="modal-title fs-5" id="editStajerModalLabel">
              <i className="fas fa-edit me-2"></i>Stajer Düzenle
            </h1>
            <button 
              type="button" 
              className="btn-close" 
...
export default EditStajerModal
